...
export const fetchUtilityBoundaries = async ({ bounds, zoom }) => {
  try {
    ...
    const response = await fetch(
      `${API_CONFIG.GAMECHANGRR_API}/utility-boundaries?bounds=${west},${south},${east},${north}&zoom=${zoom}`
    );
...
export const fetchSolarInstallations = async ({ bounds, zoom, filters }) => {
  try {
    ...
    const response = await fetch(
      `${API_CONFIG.GAMECHANGRR_API}/solar-installations?${queryParams}`
    );
...
export const fetchEVStations = async ({ bounds, zoom }) => {
  try {
    const [west, south, east, north] = bounds;
    const response = await fetch(
      `${API_CONFIG.GAMECHANGRR_API}/ev-stations?bounds=${west},${south},${east},${north}&zoom=${zoom}`
    );
...
export const fetchMoveIns = async ({ bounds, zoom }) => {
  try {
    const [west, south, east, north] = bounds;
    const response = await fetch(
      `${API_CONFIG.GAMECHANGRR_API}/move-ins?bounds=${west},${south},${east},${north}&zoom=${zoom}`
    );
...
export const fetchNeighborhoods = async ({ bounds, zoom }) => {
  try {
    const [west, south, east, north] = bounds;
    const response = await fetch(
      `${API_CONFIG.GAMECHANGRR_API}/neighborhoods?bounds=${west},${south},${east},${north}&zoom=${zoom}`
    );
